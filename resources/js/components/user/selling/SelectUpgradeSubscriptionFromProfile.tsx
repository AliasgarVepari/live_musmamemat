              ...
              <CardContent className="p-6 flex flex-col h-full">
                <div className="text-center mb-4">
                  ...
                  {(plan.description_en || plan.description_ar) && (
                    ...
                    ) : plan.discounted_price !== undefined && plan.original_price !== undefined && plan.discounted_price < plan.original_price ? (
                      <div className="flex flex-col items-center">
                        <div className="flex items-center gap-2">
                          <span className={`text-2xl font-bold text-green-600 ${language === 'ar' ? 'font-arabic' : ''}`}>
                            KD {plan.discounted_price.toFixed(2)}
                          </span>
                          <span className={`text-lg line-through text-muted-foreground ${language === 'ar' ? 'font-arabic' : ''}`}>
                            KD {plan.original_price}
                          </span>
                        </div>
                        <Badge variant="destructive" className="mt-1">
                          {language === 'ar' ? `وفر KD ${plan.discount_amount?.toFixed(2)}` : `Save KD ${plan.discount_amount?.toFixed(2)}`}
                        </Badge>
                      </div>
                    ) : (
                      <span className={`text-2xl font-bold ${language === 'ar' ? 'font-arabic' : ''}`}>
