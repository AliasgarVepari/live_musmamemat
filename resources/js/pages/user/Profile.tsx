...
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger } from '@/components/user/ui/alert-dialog';
...
function Profile({ user, requiresAuth, stats, recentListings: initialRecentListings, recentWishlist: initialRecentWishlist, subscriptionPlans, transactions }: ProfileProps) {
    ...
    const handleUpgradeSubscription = () => {
        ...
        setUpgradeData('plan_id', selectedUpgradePlan);
        upgradeSubscription('/profile/upgrade', {
            onSuccess: () => {
    ...
    const handleFeatureAd = (adId: number) => {
        router.post(`/profile/ads/${adId}/feature`, {}, {
            preserveScroll: true,
            onSuccess: () => {
                // Refetch listings to update the UI
                refetchListings();
        ...
        <div className="bg-background min-h-screen">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div>
                                                ...
                                                <Input
                                                    ...
                                                    placeholder={language === 'ar' ? 'أدخل البريد الإلكتروني (اختياري)' : 'Enter email (optional)'}
                                                />
                                            </div>
                                            <div className="grid grid-cols-2 gap-4">
                                                <div>
                                        ...
                                        {(recentWishlist?.length ?? 0) > 0 ? (
                                            ...
                                            <div key={item.id} className="flex items-center gap-3">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <div className="text-muted-foreground text-xs">
                                                            {formatPrice(item.price)} • {language === 'ar' ? 'بواسطة' : 'by'} {language === 'ar' ? item.user?.name_ar : item.user?.name_en}
                                                        </div>
                                                    </div>
                                                </div>
                                            ))
                            ...
                            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                                {(recentWishlist?.length ?? 0) > 0 ? (
                                    recentWishlist?.map((item) => (
                                        <Card 
                                            key={item.id} 
                                            className="hover:shadow-lg group transition-all duration-300 cursor-pointer"
                                        ...
                                        <CardContent className="p-4">
                                            ...
                                            <div className="text-muted-foreground text-sm">
                                                    {language === 'ar' ? 'بواسطة' : 'by'} {language === 'ar' ? item.user?.name_ar : item.user?.name_en}
                                            </div>
                                        </CardContent>
                                    </Card>
                                    ))
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <div className="flex gap-2">
                                                        ...
                                                        <Button 
                                                            ...
                                                            className={listing.is_featured ? "bg-yellow-100 text-yellow-800 hover:bg-yellow-200" : ""}
                                                        >
                                                            <TrendingUp className="mr-2 h-4 w-4" />
                                                            {listing.is_featured 
                                                                ? (language === 'ar' ? 'إلغاء التمييز' : 'Unfeature') 
                                    ...
                                    <CardContent className="space-y-4">
                                        ...
                                        <div className="space-y-3">
                                                {transactions?.map((transaction, index) => (
                                                    <div key={transaction.id || index} className="flex items-center justify-between border-b py-3">
                                                        <div className="flex-1">
            ...
            <Dialog open={isEditAdDialogOpen} onOpenChange={setIsEditAdDialogOpen}>
                <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col p-0 rounded-lg">
                    {/* Sticky Header */}
                    <DialogHeader className="flex-shrink-0 bg-background border-b p-6 rounded-t-lg">
                        <DialogTitle>
                    ...
                    {/* Sticky Footer */}
                    <div className="flex-shrink-0 bg-background border-t p-6 flex justify-end gap-2 rounded-b-lg">
                        <Button 
                            variant="outline" 
                        ...
                        <Button onClick={handleUpdateAd}>
                            {editingAd && editingAd.is_approved === false 
                                ? (language === 'ar' ? 'حفظ وإرسال للمراجعة' : 'Save and Send to Review')
            ...
            <Dialog open={showInactiveModal} onOpenChange={setShowInactiveModal}>
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        <DialogTitle>
                            {language === 'ar' ? 'رسالة الإعلان غير النشط' : 'Inactive Ad Message'}
                        </DialogTitle>
                    </DialogHeader>
                    <div className="py-4">
